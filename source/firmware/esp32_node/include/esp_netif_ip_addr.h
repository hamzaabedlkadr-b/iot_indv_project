/*
 * Compatibility shim for PlatformIO ESP-IDF builds that embed the
 * Arduino core sources locally. Some compile units do not receive the
 * upstream esp_netif public include directory, so we vendor the small
 * public header here to keep Arduino's IPAddress API available.
 */
...
#define ESP_IP_IS_ANY(addr) \
    ((addr.type == ESP_IPADDR_TYPE_V4 && ip4_addr_isany_val(addr.u_addr.ip4)) || \
