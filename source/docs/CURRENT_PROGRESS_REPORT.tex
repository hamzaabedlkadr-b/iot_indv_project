...
\begin{itemize}
    ...
    \texttt{MQTT/WiFi},
    \item publishes compact aggregate payloads through \texttt{LoRaWAN + TTN}.
\end{itemize}

The main local validation milestones are complete. Current evidence covers:

\begin{itemize}
    \item \texttt{MQTT/WiFi} edge delivery,
    \item \texttt{LoRaWAN/TTN} cloud delivery,
    \item adaptive sampling and three signal profiles,
    \item anomaly-filter evaluation and communication-volume comparison,
    \item \texttt{INA219} energy measurement on real hardware.
\end{itemize}

The only intentionally weaker item is:
...
\hline
\texttt{9} & LoRaWAN + TTN & Complete & Integrated main-app LoRaWAN validation succeeded on real hardware \\
\hline
\texttt{10} & Performance measurements & Mostly complete & Latency, payload-size, and INA219 energy evidence exist \\
\hline
...
\begin{itemize}
    \item \texttt{LoRaWAN/TTN} path is implemented and validated,
    \item serial and \texttt{TTN} evidence are saved in the repository.
\end{itemize}
...
Energy was measured with the same two-board \texttt{INA219} setup described in
the runbook and used by the reference-style repositories:

\begin{itemize}
    \item one Heltec DUT running the main FreeRTOS firmware,
    \item one Heltec monitor reading the \texttt{INA219},
    \item DUT powered only through the \texttt{INA219} high-side path,
    \item same WiFi network, \texttt{MQTT\_ONLY} mode, and
    \texttt{clean\_reference} profile for the comparable runs.
\end{itemize}

\subsection*{Baseline Versus Adaptive}

\begin{center}
\begin{tabular}{|l|r|r|r|}
\hline
\textbf{Metric} & \textbf{Baseline 50 Hz} & \textbf{Adaptive 40 Hz} & \textbf{Delta} \\
\hline
Average current & \texttt{110.4953 mA} & \texttt{110.6984 mA} & \texttt{+0.18\%} \\
\hline
Average power & \texttt{553.0000 mW} & \texttt{552.6775 mW} & \texttt{-0.06\%} \\
\hline
Integrated energy & \texttt{18.433238 mWh} & \texttt{18.422466 mWh} & \texttt{-0.06\%} \\
\hline
Peak power & \texttt{873.0000 mW} & \texttt{793.0000 mW} & \texttt{-9.16\%} \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
    \item \texttt{results/summaries/ina219\_baseline\_2026-04-21.md},
    \item \texttt{results/summaries/ina219\_adaptive\_2026-04-21.md},
    \item \texttt{results/summaries/ina219\_comparison\_2026-04-21.md},
    \item \texttt{pics/hardware.png},
    \item \texttt{pics/2026-04-21\_ina219\_adaptive\_betterserialplotter.png}.
\end{itemize}

The honest interpretation is that adaptive sampling reduces the
sample-processing rate, but WiFi, display, MQTT, and always-on FreeRTOS
activity dominate this board's electrical profile. Therefore, the measured
average energy improvement is small, while the peak power is lower in the
adaptive run.

\subsection*{Optional Deep Sleep Comparison}

An optional adaptive + deep-sleep experiment was also captured. It runs the
adaptive pipeline, then enters timed deep sleep cycles. This reduced integrated
energy to \texttt{13.632451 mWh} over \texttt{119.480 s}, about
\texttt{26.04\%} lower than the fixed \texttt{50 Hz} awake baseline.

Evidence:

\begin{itemize}
    \item \texttt{results/summaries/ina219\_deepsleep\_2026-04-21.md},
    \item \texttt{results/summaries/ina219\_three\_mode\_comparison\_2026-04-21.md},
    \item \texttt{pics/2026-04-21\_ina219\_deepsleep\_betterserialplotter.png}.
\end{itemize}

This should be presented as an additional low-power strategy rather than as
the required adaptive-sampling comparison.
...
This is exactly what the saved payload summaries and the final
communication-volume comparison show:

\begin{itemize}
    \item payload sizes remain around \texttt{444} to \texttt{454} bytes depending on the profile.
    \item fixed \texttt{50 Hz} represented \texttt{1250} samples across five aggregate windows,
    \item adaptive \texttt{40 Hz} represented \texttt{1000} samples across five aggregate windows,
    \item both modes sent five aggregate \texttt{MQTT} messages, so total aggregate payload stayed \texttt{2270 B}.
\end{itemize}
...
Evidence: \url{results/summaries/communication_volume_comparison_2026-04-21.md}.
...
\hline
RAM & \texttt{38120 B} (\texttt{11.6\%}) \\
\hline
Flash & \texttt{827875 B} (\texttt{24.8\%}) \\
\hline
\end{tabular}
\end{center}
...
\hline
\texttt{LoRaWAN/TTN} live proof & complete & serial and TTN screenshots are saved from the integrated main app \\
\hline
Energy measurements & complete & INA219 baseline/adaptive runs are saved and compared \\
\hline
Secure MQTT live TLS proof & pending & firmware support exists, but no saved live TLS run yet \\
\hline
Final short report & complete & submission snapshot and evidence matrix now capture the current state \\
\hline
\end{tabularx}
\end{center}
...
\begin{itemize}
    ...
    \item completed communication-volume comparison,
    \item completed Z-score and Hampel anomaly-filter evaluation.
\end{itemize}
...
pipeline works, the edge and cloud paths are proven, and the remaining work is
focused on optional secure-MQTT proof rather than on missing core functionality.
